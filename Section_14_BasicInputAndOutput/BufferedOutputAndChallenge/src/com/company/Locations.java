...
public class Locations implements Map<Integer, Location> {
    ...
    public static void main(String[] args) throws IOException { //specifying the ioexception
        try(BufferedWriter locFile = new BufferedWriter(new FileWriter("locations.txt")); // adding the BufferedWriter
            BufferedWriter dirFile = new BufferedWriter(new FileWriter("directions.txt"))) { // try with resources
            for (Location location: locations.values()) {
                locFile.write(location.getLocationId() + "," + location.getDescription() + "\n");
                for (String direction: location.getExits().keySet()) {
                    if (!direction.equalsIgnoreCase("Q")) { // if Q is not an option for directions
