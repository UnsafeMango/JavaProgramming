...
public class Main {
    ...
    public static void main(String[] args) {
         ...
         * (in the case of a linked tree)
         * i.e if you call your abstract class ListItem, then it would have 2 member variables of type ListItem
         * that will hold references to the next/right and previous/left variables
         *
         * The abstract class will also need to hold a value - try to be as flexible as possible
         * when defining the type of this value
         *
         * The class will also need methods to move to move to the next item and back to the previous item,
         * and methods to set the next and previous items
         *
         * You should also specify a compareTo() method that takes a parameter of the same type as the class
         * and returns 0 if the values are equal, greater than zero if the value sorts greater than the parameter
         * and less than zero if it sorts less than the parameter.
         *
         * Create a concrete class from your abstract tree item class and use this in a LinkedList class
         * to implement a linked tree that will add items in order (so that they are sorted alphabetically)
         ...
         *          current item (i.e., make the previous item's "next" refer to the new item, and the
         *          new item's "next" refers to the current item).
         * If the item to be added is greater than the current item, move onto the next item and compare
         *          again(if there is no item, then that is where the new item belongs)
         *
         ...
         *
         * Hint: if you are creating classes with the names such as List, LinkedList, Node etc, make sure that
         * you create your classes before referring to them. In previous videos we have often referred to classes
         * that we create later, but if you use names that IntelliJ recognises as Java classes (such as LinkedList)
        ...
        String[] data = stringData.split(" "); //string split method returns a string array
        // and you can specify the character that separates the elements in the array
        for (String s : data) {
